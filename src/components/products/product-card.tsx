...
import { Heart, Cpu, Monitor, Tag } from "lucide-react";
...
export function ProductCard({ product }: ProductCardProps) {
  ...
  // 🧮 1. LÓGICA DEL PRECIO (FIX DEL BUG DEL 0)
  ...
  return (
    <div className="group bg-white w-full rounded-2xl border border-slate-200 p-3 flex flex-col h-full relative hover:shadow-xl hover:border-blue-200 transition-all duration-300">
      ...
      <div className="relative aspect-[4/3] bg-slate-50 rounded-xl overflow-hidden mb-3">
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1 z-10 items-start">
          ...
          {product.condition === "NEW" && !hasDiscount && (
            <Badge className="bg-blue-600 hover:bg-blue-700 text-[10px] px-2 py-0.5 h-auto font-semibold shadow-sm">
              NUEVO
            </Badge>
          )}

          {product.condition === "REFURBISHED" && (
            <Badge className="bg-amber-500 hover:bg-amber-600 text-[10px] px-2 py-0.5 h-auto shadow-sm">
              REACONDICIONADO
            </Badge>
          )}
          ...
          {product.stock === 0 && (
            <Badge
              variant="destructive"
              className="text-[10px] px-2 py-0.5 h-auto shadow-sm"
            >
              AGOTADO
            </Badge>
          )}
      ...
      <div className="flex-1 flex flex-col min-h-[140px]">
        ...
        >
          <h3
            className="font-bold text-slate-800 text-sm leading-tight line-clamp-2 h-10"
            title={product.name}
          >
            {product.name}
          </h3>
